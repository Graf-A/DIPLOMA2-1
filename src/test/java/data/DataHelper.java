...
public class DataHelper {
    ...
    private static final String notExistCardNumber = "4444 4444 4444 4444";
    ...
    private static BankCard getBaseCard_en(String cardNumber) {
        ...
        String year = Integer.toString(faker_en.number().numberBetween(numberYear + 1, numberYear + 3)).substring(2);
    ...
    public static BankCard getApprovalCart_en() {
        return getBaseCard_en(approvedCardNumber);
    }

    // Не одобрение поездки
    public static BankCard getNotApprovalCart_en() {
    ...
    // Пустой номер карты
    public static BankCard getAnEmptynumberCart_en() {
    ...
    public static BankCard getApprovalCart_ru() {
    ...
    public static BankCard getAnInvalidCardMonth() {
        BankCard card = getBaseCard_en(approvedCardNumber);
        card.setExpireMonth(Integer.toString(faker_en.number().numberBetween(13, 20)));
        return card;
    }

    public static BankCard getAnInvalidCardMonth00() {
        BankCard card = getBaseCard_en(approvedCardNumber);
        card.setExpireMonth(("00"));
        return card;
    }

    // Получаем неверно указан формат задания месяца
    public static BankCard GettingAnInvalidCartFormat() {
        BankCard card = getBaseCard_en(approvedCardNumber);
        card.setExpireMonth(Integer.toString(faker_en.number().numberBetween(1, 6)));
        return card;
    }

    // Получаем пустое поле месяц
    public static BankCard GettingAnEmptyCart() {
        BankCard card = getBaseCard_en(approvedCardNumber);
        card.setExpireMonth("");
        return card;
    }
    ...
    public static BankCard gettingTheWrongYear() {
        ...
        card.setExpireYear(Integer.toString(faker_en.number().numberBetween(numberYear + 6, 99)));
        return card;
    }

    // Получаем неверный формат года
    public static BankCard getInvalidFormatYearCard() {
        BankCard card = getBaseCard_en(approvedCardNumber);
        card.setExpireYear(Integer.toString(faker_en.number().numberBetween(1, 9)));
        return card;
    }
    ...
    // Получаем неверно указан срок действия карты
    public static BankCard gettingAnExpiredCrd() {
        BankCard card = getBaseCard_en(approvedCardNumber);
    ...
    public static BankCard GettingInvalidCvcCode() {
        BankCard card = getBaseCard_en(approvedCardNumber);
        card.setCvcCode(faker_en.number().digits(2) + "w");
        return card;
    }
